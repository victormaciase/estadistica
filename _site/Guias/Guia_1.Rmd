...
knitr::opts_chunk$set(results='hide', message = FALSE, warning = FALSE, echo = FALSE)
```
...
```{r}
stock_total_tb <- tibble::tibble(zona = c("A", "B", "C", "D"),
                         stock_1 = c(800,200,700,300),
                         stock_2 = c(700,300,600,400)
                           )
stock_total_tb
...
```{r}
stock_total_tb %>% slice_head(n= 3)
```

```{r}
stock_total_tb %>% slice_tail(n= 2)
```
...
```{r }
readr::write_csv(stock_total_tb, file = "stock_total.csv")
```
